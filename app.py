from flask import Flask, request import requests import os import json

app = Flask(name)

DISCORD_WEBHOOK = os.getenv("DISCORD_WEBHOOK") EXPECTED_ADDRESS = os.getenv("EXPECTED_ADDRESS") ORDERS_FILE = "orders.json"

def load_orders(): try: with open(ORDERS_FILE, "r") as f: return json.load(f) except (FileNotFoundError, json.JSONDecodeError): return {}

def find_matching_order(amount): orders = load_orders() for user_id, order in orders.items(): expected = order.get("expected_total", 0) if abs(amount - expected) < 0.01: return user_id, order return None, None

@app.route('/ltc-webhook', methods=['POST']) def ltc_webhook(): data = request.json
